...
<body>
	...
	<div class="container">
		...
		<form id="create-survey-form" method="post">
			...
			<div class="form-group">
				...
				<input type="text" id="question-text" name="question_text">
			</div>
	...
	<script>
        ...
        $('#add-option').on('click', function() {
          ...
          optionHtml += '<input type="text" id="option-' + optionCount + '" name="question_options[]">';
          optionHtml += '</div>';
        ...
        $('#add-question').on('click', function() {
          ...
          questionHtml += '<input type="hidden" name="question_type_' + questionCount + '" value="' + questionType + '">';
          questionHtml += '<input type="hidden" name="question_text_' + questionCount + '" value="' + questionText + '">';
          var optionsHtml = '';
          if ($('#question-type').val() == 'multiple_choice') {
            var optionIndex = 1;
            $('.option-group input').each(function() {
              var optionLetter = String.fromCharCode(optionIndex + 64);
              optionsHtml += '<input type="hidden" name="question_options_' + questionCount + '[]" value="' + $(this).val() + '">';
              optionsHtml += '<div class="form-group">';
              optionsHtml += '<input type="radio" id="correct-answer-' + questionCount + '-' + optionIndex + '" name="correct_answer_' + questionCount + '" value="' + optionLetter + '">';
              optionsHtml += '<label for="correct-answer-' + questionCount + '-' + optionIndex + '">' + optionLetter + '.) ' + $(this).val() + '</label>';
              optionsHtml += '</div>';
              optionIndex++;
            });
          }
